...
import { Prisma, AccessToken as AccessTokenInterface, PrismaClient } from '@prisma/client';
...
type AccessTokenResolveInterface = {
  [K in NeedResolveFields]: (...args: any[]) => AccessToken[K] | Promise<AccessToken[K]>;
};
...
export class AccessTokenResolver implements AccessTokenResolveInterface {
    ...
    @Mutation(() => AccessToken, { description: "Sign In, create a access token" })
    signIn(@Args() args: SignInArgument) {
        return this.accessTokenService.signIn(args);
    }

    @Mutation(() => AccessToken, {description: 'Sign Up, create a access token' })
    signUp() {
        // TODO: implement
    }
}
