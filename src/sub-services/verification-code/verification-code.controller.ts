...
import { VerificationCodeMessage, VerificationCodeService as VerificationCodeBusinessService } from "../../business";
...
export class VerificationCodeSubServiceController {
    ...
    async send(data: StringValue, _metadata: Metadata, _call: ServerUnaryCall<StringValue, Empty>): Promise<Empty> {
        const phone = this.#verifyPhoneNumber(data.value);
        
        try {
            const message = await VerificationCodeMessage.createMessage(this.prisma, phone);
