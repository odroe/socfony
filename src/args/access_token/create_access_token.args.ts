...
export class CreateAccessTokenArgs {
   ...
   */
  @Field(() => String, { nullable: false, description: 'Account, which will be used to create access token.\naccount is username/id/phone,/email.' })
  account: string;
   ...
   */
  @Field(() => String, { nullable: false, description: 'Password, which will be used to create access token.\npassword is account password.' })
  password: string;

  /**
   * if password is one-time password.
   */
  @Field(() => Boolean, { nullable: false, description: 'if password is one-time password.' })
  otp: boolean;
}
