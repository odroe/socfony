...
class DatabaseConnectionPool {
  ...
  factory DatabaseConnectionPool.fromString(String uri) => DatabaseConnectionPool(Uri.parse(uri));

  /// Create the database connection pool.
  static Pool _createPool(Uri uri) {
    final Map<String, String> query = uri.queryParameters;
    final int maxConnections = int.parse(query['connection_limit'] ?? '${Platform.numberOfProcessors + 1}');
    final int? poolTimeout = query['pool_timeout']?.isNotEmpty == true ? int.parse(query['pool_timeout']!) : null;
    final Duration? poolTimeoutDuration = poolTimeout == null ? null : Duration(seconds: poolTimeout);
  ...
  Future<PostgreSQLConnection> _createConnection() async {  
    ...
    final bool ssl = _uri.queryParameters['ssl'] == 'true' || _uri.queryParameters['ssl'] == '1';
    final int connectTimeout = int.tryParse(_uri.queryParameters['connect_timeout'] ?? '30') ?? 30;
    final int queryTimeout = int.tryParse(_uri.queryParameters['query_timeout'] ?? '30') ?? 30;
  ...
  /// 
  /// This method returns a [Future] that completes with a [PostgreSQLConnection] object.
  Future<PostgreSQLConnection> getConnection() => _pool.withResource<PostgreSQLConnection>(_createConnection);
}
