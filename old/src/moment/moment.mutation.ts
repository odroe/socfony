...
export class MomentMutation {
  ...
  @GrpcMethod()
  async create(request: CreateMomentRequest.AsObject, metadata: Metadata) {
    const { userId } = await this.accessTokenService.verifyWithMatadata(
      metadata,
    );
    const data = this.createDatabaseData(
      await this.createMomentRequest(request),
      userId,
    );
  ...
  private createDatabaseData(
    request: CreateMomentRequest,
    userId: string,
  ): Prisma.MomentUncheckedCreateInput {
  ...
  private async createMomentRequest(
    request: CreateMomentRequest.AsObject,
  ): Promise<CreateMomentRequest> {
  ...
  private async createMedia(media?: Media.AsObject): Promise<Media> {
    ...
    if (media?.audio && media.audio.src) {
      ...
      const exists = await this.storageService.exists(media.audio.src, (data) =>
        this.validateIs('audio', data),
      );
      ...
      const exists = await this.storageService.exists(paths, (data) =>
        this.validateIs('image', data),
      );
